...
which can be written as
	\begin{equation}
		...
		\right\},
\end{equation}
such that $\begin{bmatrix}	\star \end{bmatrix}^\top$ is used as in the following.
\[
...
Then, the set of system matrices consistent with all data points can be described as follows
	...
	\end{equation}
\noindent
The set $\mathcal{C}$ can be further represented as
	\begin{equation}
...
The $\gamma$ here upper bounds the optimal cost in \eqref{eq:main_problem} without considering the input and state constraints.

To incorporate the remaining constraints, they can be expressed as the following LMIs \eqref{eq:constraints_LMIs} \cite{xie2024}.
...
Now, having \eqref{SDP_LMIs} and \eqref{eq:constraints_LMIs}, the data-driven min-max MPC \eqref{eq:main_problem} can be solved after representing it in its SDP formulation as the following.
\begin{subequations} \label{eq:final_gamma}
	\begin{gather}
		\minimise_{\gamma, H, L, \tau} \gamma \\
		\text{s.t.} \quad \eqref{SDP_LMIs} \text{ and } \eqref{eq:constraints_LMIs} \text{ hold.}
	\end{gather}
\end{subequations}
...
\begin{framed}
	...
	\]
	given an initial state $x_t$ subject to some unput and state constraints and the corresponding consistent set of system matrices \(\mathcal{C}\).
