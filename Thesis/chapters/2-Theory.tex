...
\todo[inline]{Add the Fork Model simple drawing}
	...
	\end{equation}
	where $\varphi$ is the lean angle, $\delta$ is the steer angle, $g$ is the gravity acceleration, $h$ is the height of the center of mass, $l$ is the wheelbase, $a$ is the distance from the center of mass to the front wheel, $v$ is the speed of the bicycle, and $r_\tau$ is the radius of the tire. The bicycle model is a second-order system with two states: lean angle $\varphi$ and steer angle $\delta$.
...
Let \((A, B)\) be the set of system matrices that satisfy the data \(x_i,\ u_i,\ x_{i+1};\ i \in \mathbb{N}\), corresponding to an index in the sequence of data which consists of inputs, noise, and state measurements. This set of system matrices can be written as follows
	\begin{equation}
		\Sigma_i := \left\{ ( A, B ) : (\ref{eq:system}) \text{ holds for } \omega_i \text{ satisfying } \|\omega_i\|_2^2 \leq \epsilon \right\},
	\end{equation}
...
The set $\mathcal{C}$ can be further represented as
	\begin{equation}
		...
		\forall \tau = (\tau_0, \ldots, \tau_{T-1}),\, \tau_i \geq 0,\, i \in \mathbb{I}_{[0,T-1]}
		\right\},
	\end{equation}
such that $\Pi$ is
\begin{equation}
...
\begin{subequations} \label{eq:main_problem}
	\begin{align}
		J_\infty^*(x_t) &:= \min_{\bar{u}(t)} \max_{(A, B) \in \mathcal{C}} \sum_{k=0}^{\infty} l(\bar{u}_k(t), \bar{x}_k(t)) \\
		...
		& \|\bar{x}_k(t)\|_{S_x} \leq 1, \ \forall (A, B) \in \mathcal{C}, \ t \in \mathbb{N} \label{subeq:e}.
	\end{align}
\end{subequations}
...
Now, having \eqref{SDP_LMIs} and \eqref{eq:constraints_LMIs}, the data-driven min-max MPC \eqref{eq:main_problem} in its SDP formulation can be solved after representing as the following.
...
The optimal solution for problem \eqref{eq:final_gamma} is then represented as $\gamma^\star, \, H^\star, \, L^\star, \, \tau^\star$. Consequently, the optimal state-feedback gain is $F = L^\star (H^\star)^{-1}$
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE Solve the problem \eqref{20};
