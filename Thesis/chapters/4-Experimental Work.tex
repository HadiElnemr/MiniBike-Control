...
\section{Mini-Bike}
The mini-bike used in the experiments based on the \textbf{WHEELTEC Z100 Servo Self-Balancing Bicycle} in Figure \ref{fig:minibike}. It features an STM32F103C8T6 microcontroller as the control unit, which utilises different sensors and motor control circuits to achieve stabilisation.
A similar bicycle was studied in \cite{tapken2018modelling} using multibody software modelling.
...
The bike uses steering angle control to self-balance. This is done through an control gain, originally from LQR, followed by a PID controller. The LQR computes the state-feedback gain that minimises a cost function. While the PID controller adjusts the PWM signals to the servo motor to fine-tune the steering speed.

The bike is balanced continuously through getting data from the gyroscope, which provides real-time data on tilt angles and angular velocities. The measurements are processed by the microcontroller to compute the control law, where the input is the steering angle derivative. The microcontroller is configured to send control inputs to the servo at a frequency of 50 Hz.
...
The platform on which the development of codes were done is the \textit{STM32Cube IDE}, which is a free unlimited software for writing and compiling the codes. It compiles the codes into a Hexadecimal file which can then be flashed on the STM32 microcontroller using FlyMCU software.
...
The data-driven MPC controller was not implemented since the real-life errors prevented successful solving of the SDP on real data as stated before. Due to this limitation, the SDP optimisation problem was solved offline, and the resulting state-feedback gain was coded into the embedded controller as a static gain.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{Experiment} & \textbf{Controller} & \textbf{Fork Angle}  & \textbf{Target Angle} \\ \hline
...
The results acquired on implementing the controllers and collecting the data are plotted in \autoref{fig:experiments}. It can be noticed that the data length varies across the experiments. This is due to the varying capability of collecting the data before the bicycle falls, particularly in experiment 3 in \autoref{fig:exp3}, where the controller did not maintain bicycle balance for an extended period. In \autoref{fig:exp4} of Experiment 4, it is noticed that the data-driven state-feedback on the fork angle bicycle model achieved balancing of the bicycle comparable to that of the LQR. Figures \ref{fig:experiments} show that the average lean angle achieved differs from the target angle. This can arise from different factors including the external noise and inaccuracies in measuring the potentiometer value responsible for setting the target angle.
...
\begin{figure}[htbp]\ContinuedFloat
	...
	\begin{subfigure}[b]{\textwidth}
		...
		\caption{\boldmath Data-driven state-feedback for $75^\circ$ fork angle and $4^\circ$ target angle.}
				\label{fig:exp4}
	\end{subfigure}
	\caption{\boldmath Lean angle over time under different data-driven state-feedback settings for an initial state vector (in radians) $x_0 = [0.0873, \, 0, \, 0]^\top$, $Q = \operatorname{diag}(300,\,0.1,\,300)$, $R = 1$, and noise bound $\epsilon = 0.001$ for data synthesis of length $T = 40$. The data is smoothed using a rolling window which takes the mean of each 20 data points. The average lean angle achieved is also shown for each experiment.}
